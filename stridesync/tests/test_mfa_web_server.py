...
def test_format_zone_range_open_ended_top_zone():
    assert mfa_web_server._format_zone_range(175, None) == "175+ bpm"


def test_format_zone_range_bounded_by_next_zone():
    assert mfa_web_server._format_zone_range(130, 150) == "130–149 bpm"


def test_format_zone_range_handles_missing_value():
    assert mfa_web_server._format_zone_range(None, 150) == "—"


def test_hr_zone_ranges_returns_empty_when_db_missing(tmp_path):
    assert mfa_web_server._hr_zone_ranges(str(tmp_path / "no_such.db")) == []


def test_hr_zone_ranges_reads_from_most_recently_synced_activity(tmp_path):
    from app import db
    ...
    conn.execute(
        ...
        INSERT INTO activities (
            ...
            (1, 'Older run', 'running', datetime('now', '-5 days'), 'x', 5000.0,
             datetime('now')),
            (2, 'Most recent run', 'running', datetime('now', '-1 days'), 'x', 5000.0,
             datetime('now'))
        """
    )
    conn.execute(
        ...
        VALUES
            (1, 1, 90, 600.0), (1, 2, 120, 1200.0),
            (2, 1, 100, 600.0), (2, 2, 130, 1200.0), (2, 3, 150, 300.0), (2, 4, 165, 0.0),
            (2, 5, 175, 0.0)
        """
    )
    conn.commit()
    ...
    zones = mfa_web_server._hr_zone_ranges(db_path)

    # Reads boundaries from activity 2 (most recently synced), not the stale ones on activity 1.
    assert [(z["zone_number"], z["zone_low_boundary_hr"]) for z in zones] == [
        (1, 100),
        (2, 130),
        (3, 150),
        (4, 165),
...
def test_running_page_shows_hr_zone_ranges_before_weekly_distance(tmp_path):
    from app import db
    ...
    assert "100–129 bpm" in response.text
    assert "130+ bpm" in response.text
    assert "Weekly total distance" in response.text
    # The zone ranges are positioned before the weekly mileage list, per the requested layout.
